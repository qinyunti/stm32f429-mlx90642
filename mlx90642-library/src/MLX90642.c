...
int MLX90642_Init(uint8_t slaveAddr)
{
    ...
    if(status < 0)
        return status;
    xprintf("RefreshTime=%d\r\n",status);
    ref_time = (uint16_t)status;

#if 0
    do{
        status = MLX90642_GetProgress(slaveAddr);
        if(status < 0){
            xprintf("ClearDataReady=%d\r\n",status);
            return -MLX90642_INVAL_VAL_ERR;
        }
        xprintf("GetProgress=%d\r\n",status);
        if(status < 100){
